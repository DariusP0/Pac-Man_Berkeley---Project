...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        """
        Update the Q-value for the given state-action pair based on the observed transition.
        """
        sample = reward + self.discount * self.computeValueFromQValues(nextState)
        self.QValues[(state, action)] = self.getQValue(state, action) - self.alpha * self.getQValue(state, action) - self.alpha + self.alpha * sample
